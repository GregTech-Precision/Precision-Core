...
import precisioncore.common.metatileentities.multi.parallel.CommunicationTower;
import precisioncore.common.metatileentities.multi.parallel.SatelliteHatch;
...
public class PrecisionParallelAPI {
        ...
        if(netAddress != null && location != null) {
            if (!communicationTowers.isEmpty() && communicationTowers.containsKey(netAddress) && !communicationTowers.get(netAddress).isEmpty()){
                for(CommunicationTower tower : communicationTowers.get(netAddress)){
                    ...
                    if(PrecisionUtility.getDistance(towerPos, location) <= 256){
                        return tower;
                    }
                }
            }
